...
Użycie pakietu listings ma też kolejną zaletę nad pakietem minted. 
Nie wymaga uruchomienia dodatkowego procesu Pythona w trakcie kompilacji dokumentu (włączanego przełącznikiem \verb|-shell-escape|) z zainstalowanym pakietem PyPi \verb|pygmentize|.
Ponadto może zostać użyty do definiowania własnych języków programowania w prosty sposób:
\begin{verbatim}
\lstdefinelanguage{MojJezyk}{
   keywords={int,float,if,else,while,do,end},
   basicstyle={\ttfamily},
   morestring=[b]"
}
\end{verbatim}
Więcej opcji konfiguracji można znaleźć w dokumentacji pakietu listings -- punkt 3.2 Language definitions (w terminalu wpisać \verb|texdoc -l listings| i podać numer pliku listings.pdf).
...
pozycji w~tekście.
Dopuszcza się również stosowanie Bib\TeX{}a i jego standardowego stylu bibliografii o~nazwie \verb|plain|.
...
Stronę tytułową oraz stronę z podziękowaniem pozostawia się bez widocznego numeru strony.
Aby zmienić styl tylko bieżącej strony na niemający numeru należy skorzystać z polecenia \verb|\thispagestyle{empty}|.
